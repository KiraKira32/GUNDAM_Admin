          ...
          <template v-for="order in orders" :key="order.id">
            <tr>
              <td class="order-text-content">
                {{ $moment(new Date(order.create_at * 1000)).format('YYYY-MM-DD') }}
              </td>
              <td>
                <span v-if="order.user" v-text="order.user.email" class="order-text-content"></span>
              </td>
              <td class="order-text-content">
              ...
              <td class="text-right order-text-content">
              ...
              <td>
                ...
                <label class="form-check-label" :for="order.id">
                  <span v-if="order.is_paid" class="text-primary fw-bolder order-text-content"
                    >已付款</span
                  >
...
export default {
  ...
  methods: {
    // 取得訂單列表
    getOrders(page = 1) {
      const api = `${import.meta.env.VITE_API}/v2/api/${
    ...
    updatePaid(item) {
      ...
      const api = `${import.meta.env.VITE_API}/v2/api/${import.meta.env.VITE_PATH}/admin/order/${
        item.id
      }`
      this.$http
        .put(api, { data: paid })
